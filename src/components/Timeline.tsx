...
export function Timeline({ schedule, country }: Props) {
  ...
  return (
    <ol className="relative flex flex-col gap-8 pl-2">
      <span className="absolute bottom-3 left-[27px] top-3 w-px border-l-2 border-dashed border-paper-line" />
      {schedule.map((day, i) => {
        ...
        return (
          <li key={`${day.dayNumber}-${i}`} className="relative flex gap-5">
            <div className="relative z-10 flex h-14 w-14 flex-none items-center justify-center rounded-full border-2 border-paper bg-sky-light text-sky-dark shadow-soft">
              <Icon className="h-6 w-6" />
            </div>
            <div className="flex-1 rounded-2xl border border-paper-line bg-paper/70 p-4 shadow-soft transition-shadow hover:shadow-lift sm:p-5">
